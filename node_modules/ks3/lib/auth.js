...
var hmacMd5=function(encodedStr){
...
var generateToken = function(sk, req, body) {
	...
	const md5 = req.headers && req.headers['Content-MD5']
	var content_md5 = md5 || ((!!req.body)?hmacMd5(req.body):'');
	...
	if(config.isDebug) console.log('string2Sign:', string2Sign);
...
var generateAuth = function(ak,sk,req, body) {
	...
	const auth = 'KSS '+ak+':'+ token;
	if (config.isDebug) console.log('auth: ', auth)
	return auth;
};
 ...
 * 通过 POST请求的表单实体发送的签名
 * @param sk    {string}
 * @param policy { Object或者String}  ex.
 * {
...
var getFormSignature = function(sk, policy) {
	if (Object.prototype.toString.call(policy) === '[object Object]') {
		policy = JSON.stringify(policy)
	}
	var stringToSign = new Buffer(policy).toString('base64');
